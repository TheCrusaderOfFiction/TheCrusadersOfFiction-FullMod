...
public class EnchantedSwordProjectileRenderer extends EntityRenderer<EnchantedSwordProjectileEntity> {
    public static final ResourceLocation TEXTURE = new ResourceLocation(CrusadersOfFiction.MOD_ID, "textures/entity/enchantsword1.png");
    protected EnchantedSwordProjectileModel model;

    public EnchantedSwordProjectileRenderer(EntityRendererProvider.Context pContext) {
        super(pContext);
        model = new EnchantedSwordProjectileModel(pContext.bakeLayer(ModModelLayers.ENCHANTED_SWORD_PROJECTILE_LAYER));
        this.shadowRadius = 0.5f;
    }

    public void render(EnchantedSwordProjectileEntity entity, float pEntityYaw, float pPartialTick, PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight) {
        pPoseStack.pushPose();
    ...
    @Override
    public ResourceLocation getTextureLocation(EnchantedSwordProjectileEntity pEntity) {
        return TEXTURE;
    }
}
