...
public class BlockCompressorRecipeBuilder implements RecipeBuilder {

    public static final int INPUT_COUNT = 1; // Only 1 input slot now
    ...
    public BlockCompressorRecipeBuilder(ItemLike result, int count) {
    ...
    public BlockCompressorRecipeBuilder setIngredient(Ingredient ingredient) {
        return setIngredient(0, ingredient, 1);
    }

    /** Ingredient + count */
    public BlockCompressorRecipeBuilder setIngredient(Ingredient ingredient, int count) {
        return setIngredient(0, ingredient, count);
    }

    /** ItemLike only (count = 1) */
    public BlockCompressorRecipeBuilder setIngredient(ItemLike item) {
        return setIngredient(0, Ingredient.of(item), 1);
    }

    /** ItemLike + count */
    public BlockCompressorRecipeBuilder setIngredient(ItemLike item, int count) {
        return setIngredient(0, Ingredient.of(item), count);
    }

    public BlockCompressorRecipeBuilder setIngredient(int slot, Ingredient ingredient, int count) {
        checkSlot(slot);
    ...
    /** Only change count */
    public BlockCompressorRecipeBuilder setAmount(int count) {
        checkSlot(0);
        amounts[0] = Math.max(0, count);
        return this;
    }
    ...
    public static class Result implements FinishedRecipe {
        ...
        public void serializeRecipeData(JsonObject json) {
            ...
            if (!ingredients.get(0).isEmpty()) {
                entry.add("ingredient", ingredients.get(0).toJson());
            } else {
                entry.add("ingredient", new JsonObject());
            }
            entry.addProperty("count", amounts[0]);
            array.add(entry);
        ...
        public RecipeSerializer<?> getType() {
            return BlockCompressorRecipe.Serializer.INSTANCE; // Your serializer
        }
